...
interface IMigratorChef {
    // Perform LP token migration from legacy CronaSwap to CronaSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to CronaSwap LP tokens.
    // CronaSwap must mint EXACTLY the same amount of CronaSwap LP tokens or
    // else something bad will happen. Traditional CronaSwap does not
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //   1. The pool's `accCronaPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. CRONAs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that CRONAs distribution occurs.
    ...
    function pendingCrona(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 cronaReward = multiplier.mul(cronaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 cronaReward = multiplier.mul(cronaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        crona.mint(devaddr, cronaReward.div(10));
